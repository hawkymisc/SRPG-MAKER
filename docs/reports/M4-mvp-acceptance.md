...
| 項目 | 状態 |
|------|------|
| 勝利条件UI 4種 + 敗北条件 | ✅ マップタブに追加 |
| マップ: 矩形/塗りつぶし/bottom+topレイヤー | ✅ |
| 増援UI（追加/削除） | ✅ 最小実装 |
| 無敵デバッグの戦闘反映 | ✅ `BattleSession.apply({ playerInvincible })` |
| HTML5書き出しエラーメッセージ | ✅ runtime 未起動時に案内 |
| objectレイヤー / イベント演出の本格化 | 未着手（フェーズ2） |
